...
|Користувач|
    start
    :Користувач вводить ключові слова або параметри пошуку;
    :Користувач натискає на кнопку "Пошук";
|Система|
    :Система обробляє запит користувача;
    :Система перевіряє чи користувач авторизований;
    if (Користувач не авторизований?) then (так)
        :Вивести повідомлення про необхідність авторизації;
        stop
    else (ні)
        :Система відображає результати пошуку;
        :Система вносить результати в історію пошуку;
        note left #FF736E
...
|Користувач|
    ...
    :Користувач взаємодіє з результатами;
    stop;
@enduml
```
...
|Користувач|
    start
    :Користувач переглядає результати пошуку;
    :Користувач активує функцію фільтрації;
|Система|
    :Система відображає доступні параметри фільтрації;
|Користувач|
    :Користувач обирає бажані критерії фільтрації;
|Система|
    :Система застосовує обрані фільтри до результатів пошуку;
    :Система перевіряє чи користувач авторизований;
    if (Користувач не авторизований?) then (так)
        :Вивести повідомлення про необхідність авторизації;
        stop
    else (ні)
        :Система відображає оновлені результати з врахуванням застосованих фільтрів;
        note left #FF736E
...
|Користувач|
    :Користувач переглядає оновлені результати з врахуванням застосованих фільтрів;
    stop;
@enduml
```
