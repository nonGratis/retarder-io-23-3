...
| ***ВИКЛЮЧНІ СИТУАЦІЇ***: | Невірний логін EX.Visitor.Login.WrongLogin<br /> Невірний пароль EX.Visitor.Login.WrongPass<br /> Блокування облікового запису Ex.Visitor.Login.AccBlocked<br />Обліковий запис не існує Ex.Visitor.Login.AccDoesntExist<br /> Невірна електронна пошта EX.Visitor.Login.WrongEmail |
...
|Система|
    :Перевіряє дані з форми;
        note right #FF736E
            EX.Visitor.Login.WrongLogin
            EX.Visitor.Login.WrongEmail
            EX.Visitor.Login.WrongPass 
            Ex.Visitor.Login.AccBlocked
            Ex.Visitor.Login.AccDoesntExist
        end note
...
|Відвідувач|
    start
    :Починає взаємодію;
    :Натискає кнопку видалення облікового запису;
|Система|
    :Надає форму для заповнення;
|Відвідувач|
...
|Система|
    :Перевіряє дані з форми;
        note right #FF736E
            User.DeleteRequest.NotConfirmed
            User.DeleteRequest.WrongPass
        end note
...
| ***ВИКЛЮЧНІ СИТУАЦІЇ***: | Адміністратор не має доступу до редагування профілю wrong_access_rights<br />Обраного користувача не існує user_not_found<br />Адміністратор не підтверджує зміни облікового запису failure_confirm |

```plantuml
@startuml
|Адміністратор| 
    start
    :Знаходить користувача, чий профіль потрібно редагувати;
    :Натискає на кнопку "Редагувати профіль";
        note left #ffaaaa
            wrong_access_rights
        end note
|Система|
    :Відкриває форму з даними профілю користувача;
        note right #ffaaaa
            user_not_found
        end note
|Адміністратор| 
...
| ***ВИКЛЮЧНІ СИТУАЦІЇ***: | Адміністратор не має доступу до видалення облікового запису wrong_access_rights<br />Обраного користувача не існує user_not_found<br />Адміністратор не підтверджує видалення облікового запису failure_confirm |

```plantuml
@startuml
|Адміністратор| 
    start
    :Знаходить користувача, чий обліковий запис потрібно видалити;
        note left #ffaaaa
            user_not_found
        end note
...
|Система|
    :Відкриває вікно з попередженням про видалення профілю;
|Адміністратор| 
